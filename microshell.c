...
void d_putstr(int fd, const char *s) {if (s) {write(fd, s, str_len(s));}}
...
void add_cmd(char *cmds[], char *cmd) {
	for (int i = 0; i < N; i++) {
		if (!cmds[i]) {cmds[i] = cmd; break;}
	}
}

int cmds_len(char **cmds) {
	for (int i = 0; i < N; i++) {if (!cmds[i]){return i;}}
	return N;
}
...
void executor(char *cmd, char **cmds, int *i_pipe, int *o_pipe, char **ep) {
	...
	if (pid < 0) {fatal();}
	if (!pid) {
		if (i_pipe && dup2(i_pipe[PIPE_READ], STDIN_FILENO) < 0) {fatal();}
		close_pipe(i_pipe);
		if (o_pipe && dup2(o_pipe[PIPE_WRITE], STDOUT_FILENO) < 0) {fatal();}
		close_pipe(o_pipe);
		execve(cmd, cmds, ep);
